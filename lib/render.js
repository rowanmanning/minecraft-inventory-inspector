...
function renderInventoryItemAsMarkdown(item) {
	const enchantmentIndicator = (item.tag && item.tag.Enchantments && item.tag.Enchantments.length ? '📖 ' : '');
	const damageIndicator = (item.tag && item.tag.Damage && item.tag.Damage > 0 ? '🤕 ' : '');
	let customName = '';
...
function minecraftItemIdToName(id) {
	return title(id.replace(/^minecraft:/, '').replace(/_/g, ' '));
}
